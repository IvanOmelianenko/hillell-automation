...
export class AddExpenseForm extends BaseForm {
    ...
    public readonly reportDataField = this.page.locator('#addExpenseDate');
    public readonly mileageField = this.page.locator('#addExpenseMileage');
    public readonly numberOfLitersField = this.page.locator('#addExpenseLiters');
    ...
    public readonly lessOrEqualMileageErrorMessage = this.page.locator('.alert-danger', {hasText: 'First expense mileage must not be less or equal to car'});
    ...
    async addNewExpense(mileage: string, numberOfLiters: string, totalCost: string, vehicle?: string, reportDate?: string) {
        ...
        if (reportDate) {
            await this.enterReportData(reportDate);
        }
    ...
    async enterReportData(reportDate: string) {
        await this.reportDataField.fill(reportDate);
    }
